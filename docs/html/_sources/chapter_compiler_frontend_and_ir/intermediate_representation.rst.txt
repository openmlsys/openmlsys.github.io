...
.. _intermediate_representation:

.. figure:: ../img/ch04/中间表示-中间表示结构.svg
   :width: 800px
...
.. container:: {#tab:ch04/ch04-categorize}
...
.. figure:: ../img/ch04/中间表示-线性中间表示.svg
   :width: 800px
...
AST抽象语法树采用树型中间表示的形式，是一种接近源代码层次的表示。对于表达式\ :math:`a*5+a*5*b`\ ，其AST表示如图:numref:`AST_DAG`\ 所示。可以看到，AST形式包含\ :math:`a*5`\ 的两个不同副本，存在冗余。在AST的基础上，DAG提供了简化的表达形式，一个节点可以有多个父节点，相同子树可以重用。如果编译器能够证明\ :math:`a`\ 的值没有改变，则DAG可以重用子树，降低求值过程的代价。

.. _AST_DAG:

.. figure:: ../img/ch04/中间表示-ASTDAG.svg
   :width: 600px
...
.. figure:: ../img/ch04/中间表示-LLVMIR.svg
   :width: 600px
...
.. figure:: ../img/ch04/中间表示-torchscript.png
   :width: 800px

   Python代码及输出的TorchScript IR
...
.. figure:: ../img/ch04/中间表示-Jaxpr.png
   :width: 800px
...
.. figure:: ../img/ch04/中间表示-MindIR.svg
   :width: 800px

   MindIR文法。MindIR中的ANode对应于ANF的原子表达式，ValueNode用于表示常数值，ParameterNode用于表示函数的形参，CNode则对应于ANF的复合表达式，表示函数调用
...
.. figure:: ../img/ch04/中间表示-MindIR示例.PNG
   :width: 600px
...
.. figure:: ../img/ch04/中间表示-MindIR图.png
   :width: 800px
